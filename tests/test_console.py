#!/usr/bin/python3
"""Modulle for TestHBNBCommand class."""
...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_help(self):
        ...
        s = """
Documenteed commands (type help <topic>):
========================================
...
"""
    ...
    def create_class(self, classname):
        """Creates class for console tests."""
        with patch('sys.stdout', new=StringIO()) as f:
            HBNBCommand().onecmd("create {}".format(classname))
        uid = f.getvalue()[:-1]
        self.assertTrue(len(uid) > 0)
    ...
    def help_load_dict(self, rep):
        """Helper method that tests dictionary equality."""
        rex = re.compile(r"^\[(.*)\] \((.*)\) (.*)$")
        res = rex.match(rep)
    ...
    def attributes(self):
        """Returns all valid attributes and their types for classname."""
        attributes = {
            "BaseModel":
                     {"id": str,
